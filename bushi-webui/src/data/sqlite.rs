...
impl SqliteRepository {
    ...
    async fn interact<T, F>(&self, operation: &'static str, f: F) -> Result<T, DomainError>
    where
        T: Send + 'static,
        F: FnOnce(&rusqlite::Connection) -> rusqlite::Result<T> + Send + 'static,
    {
        self.interact_domain(operation, move |conn| f(conn).map_err(DomainError::from))
            .await
    }

    async fn interact_domain<T, F>(&self, operation: &'static str, f: F) -> Result<T, DomainError>
    where
        T: Send + 'static,
        F: FnOnce(&rusqlite::Connection) -> Result<T, DomainError> + Send + 'static,
    {
        let conn = {
            let _timer = crate::debug_timing::start("sqlite.pool_wait");
            self.pool.get().await?
        };
...
impl RepositoryPort for SqliteRepository {
    async fn list_repositories(&self) -> Result<Vec<RepositoryRecord>, DomainError> {
        self.interact("sqlite.list_repositories", |conn| {
    ...
    async fn get_repository(&self, name: &str) -> Result<Option<RepositoryRecord>, DomainError> {
        let cached = {
            let _timer = crate::debug_timing::start("cache.get_repository");
            self.repo_cache.get(name).await
        };
        if let Some(repo) = cached {
            return Ok(Some(repo));
        }
        let name = name.to_string();
        let repo = self
            .interact("sqlite.get_repository", move |conn| {
                let mut stmt = conn.prepare_cached(
                    "SELECT repository_id
                          , repository_name
                          , repository_path
    ...
    async fn default_rev(&self, repo: &RepositoryRecord) -> Result<String, DomainError> {
        ...
        let repo_id = repo.id;
        self.interact("sqlite.default_rev", move |conn| {
            let mut stmt = conn.prepare_cached(
                "SELECT show_name
                   FROM refs
            ...
            let tag: Option<String> = stmt
                ...
                .optional()?;
            let rev = tag
                .map(|name| format!("tag/{}", name.replace('/', ":")))
    ...
    async fn ref_time(&self, repo_id: i64, rev: &str) -> Result<Option<i64>, DomainError> {
        ...
        self.interact("sqlite.ref_time", move |conn| {
            let mut stmt = conn.prepare_cached(
                "SELECT ref_time
                   FROM refs
                  WHERE repository_id = ?1
                    AND ref_type = ?2
                    AND show_name IN (?3, ?4)
                  LIMIT 1;",
            )?;
            let exact: Option<i64> = stmt
                .query_row(
                    params![repo_id, ref_type, show_name, show_name.replace(':', "/")],
                    |row| row.get(0),
                )
                .optional()?;
            if exact.is_some() {
    ...
    async fn list_refs(&self, repo_id: i64, ref_type: i64) -> Result<Vec<RefRecord>, DomainError> {
        self.interact("sqlite.list_refs", move |conn| {
            let mut stmt = conn.prepare_cached(
                "SELECT r.show_name
                      , c.commit_hash
                      , r.ref_time
            ...
            let rows = stmt.query_map(params![repo_id, ref_type], |row| {
                Ok(RefRecord {
                    show_name: row.get::<_, String>(0)?.replace('/', ":"),
    ...
    async fn log(
        ...
        self.interact_domain("sqlite.log", move |conn| {
    ...
    async fn path_history(
        ...
        let start_commit_hash = start_commit_hash.to_string();
        let after = after.map(|s| s.to_string());
        self.interact_domain("sqlite.path_history", move |conn| {
...
impl SqliteRepository {
    async fn resolve_ref(
        ...
        self.interact("sqlite.resolve_rev", move |conn| {
            let mut stmt = conn.prepare_cached(
                "SELECT c.commit_hash
                   FROM refs AS r
                   JOIN commits AS c
    ...
    async fn resolve_hash(&self, repo_id: i64, prefix: &str) -> Result<ResolvedRev, DomainError> {
        ...
        let like = format!("{prefix}%");
        let rows = self
            .interact("sqlite.resolve_rev", move |conn| {
                let mut stmt = conn.prepare_cached(
                    "SELECT commit_hash
                       FROM commits
                      WHERE repository_id = ?1
